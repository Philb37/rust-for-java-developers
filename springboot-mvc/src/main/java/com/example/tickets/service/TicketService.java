...
import com.example.tickets.repository.StatCount;
...
public class TicketService {
    ...
    public List<TicketResponse> list(TicketStatus status, Priority priority) {
        ...
        // LIMIT 100 via the fluent query (no extra count query), matching the WebFlux app.
        return repository.findBy(spec, query -> query.limit(100).all())
                .stream()
                .map(TicketResponse::from)
                .toList();
    }
    ...
    // Grouped in SQL via the repository's GROUPING SETS query (was previously done in memory,
    // which is what the Rust side mirrors for the talk).
    public StatsResponse stats() {
        ...
        for (StatCount row : repository.countByStatusAndPriority()) {
            if (row.getStatus() != null) {
                byStatus.put(TicketStatus.valueOf(row.getStatus()), row.getCount());
            } else if (row.getPriority() != null) {
                byPriority.put(Priority.valueOf(row.getPriority()), row.getCount());
            }
        }
